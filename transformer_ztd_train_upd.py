...
def plot_and_loss(eval_model, data_source, epoch, scaler, station):
    ...
    df_sufix = pd.DataFrame(
        {'pred': np.array(test_result[samples:]).reshape(-1), 'truth': np.array(truth[samples:]).reshape(-1)})
    df.to_csv(base_dir + 'result_data/' + station[:-3] + 'data%d.csv' % epoch)
    df_prefix.to_csv(base_dir + 'result_data/' + station[:-3] + 'data_prefix%d.csv' % epoch)
    df_sufix.to_csv(base_dir + 'result_data/' + station[:-3] + 'data_sufix%d.csv' % epoch)
    ...
    plt.savefig(base_dir + 'fig/' + station[:-3] + 'transformer-epoch%d.png' % epoch)
    plt.close()
    return total_loss / i
...
if __name__ == '__main__':
    ...
    base_dir = '/home/zhanghuan/dat01/transformer/'
    stations = os.listdir(base_dir + 'data/')
    for station in stations:
        filepath = base_dir + station
        ...
        torch.save({
            ...
            'loss': val_loss
        }, base_dir + 'model/' + station[:-3] + '_model_%d.pkl' % epochs)
