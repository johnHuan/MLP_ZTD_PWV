...
def plot_and_loss(eval_model, data_source, epoch, scaler):
    ...
    df = pd.DataFrame({'test': np.array(test_result).reshape(-1), 'truth': np.array(truth[:samples]).reshape(-1)})
    df.to_csv('../transformer_model/ztd/data/data%d.csv' % epoch)
    ...
    plt.savefig('../transformer_model/ztd/fig/transformer-epoch%d.png' % epoch)
    plt.close()
    return total_loss / i
...
if __name__ == '__main__':
    ...
    filepath = 'D:/lstm/gnss/lstm/period_model/used/data/WTZ3.csv'
    # series = pd.read_csv(filepath, header=0, parse_dates=True, squeeze=True, usecols=['residual_serial'])
    series = pd.read_csv(filepath, header=0, parse_dates=True, squeeze=True, usecols=['3'])

    # normalizing input values crucial for the model
    scaler = MinMaxScaler(feature_range=(-1, 1))
    amplitude = scaler.fit_transform(series.to_numpy().reshape(-1, 1)).reshape(-1)
    #    samples = 365 * 24 * 2  # the data from 2013~2018 6 years in total,  4 years used for training, last 2 year used for test total:45249; train:[:35054]; test[35054:];
    ...
    # create transformer model in and output sequence
    ...
    for epoch in range(1, epochs + 1):
        ...
        train(train_data)
        if epoch % 2 is 0:
            val_loss = plot_and_loss(model, valid_data, epoch, scaler)
            # 模型保存
            torch.save({
                'epoch': epoch,
                ...
                'loss': val_loss
            }, '../transformer_model/ztd/model/model_%d.pkl' % epoch)
            ...
            model.train()
            """
            predict_future(model, valid_data, 200, scaler)
        else:
